...
MsgBox % inst.pageid "`n" inst.pagews
...
Return ; xxx xxx xxx xxx xxx xxx xxx xxx xxx xxx xxx xxx xxx xxx xxx xxx xxx 
...
class Chrome
{
	...
	__New(ProfilePath:="", URLs:="about:blank", Flags:="", ChromePath:="", DebugPort:="")
	{
		...
		if(this.GetPort()==this.DebugPort)
		{
			...
			this.PID := ChPID
			this.page := this.find({url:URL})	;this.page := this.GetID(URL)
			if !this.pageid
				this.page := this.NewTab(URL)
		}
		else
		{
			if (ChromePath == "")
	...
	find(obj, fnCallback:="")
	{
		...
		for k,val in this.GetPageList() {
			if obj.url && instr(val.url, StrReplace(StrReplace(obj.url,"http://"),"https://")) {
				this.pageid := val.id
				this.pagews := val.webSocketDebuggerUrl
			}
			else if obj.index && instr(val.type, "page") && obj.index=index++ {
				this.pageid := val.id
				this.pagews := val.webSocketDebuggerUrl
			}
		}
		return this.pageid ? new this.Page(this.pagews, fnCallback) : False
	}
	GetBy(obj, fnCallback:="")
	{
		if obj.Port
			this.DebugPort := obj.Port
		if(this.GetPort()==this.DebugPort){
			for k,val in this.GetPageList() {
				if obj.id && instr(val.id, obj.id) {
					this.pageid := val.id
					this.pagews := val.webSocketDebuggerUrl
				}
				else if obj.url && instr(val.url, StrReplace(StrReplace(obj.url,"http://"),"https://")) {
					this.pageid := val.id
					this.pagews := val.webSocketDebuggerUrl
				}
			}

...
if WinExist("ahk_exe chrome.exe")
{
	...
	Page.Call("Page.navigate", {"url": url2})
	Page.WaitForLoad()
}
else
{
...
class Chrome{
	...
	GetID(index, DebugPort)
	{
		n :=1
		if(this.FindPort(DebugPort)){
			this.DebugPort := DebugPort
			for k,val in this.GetPageList()
			{
				if(instr(val.type, "page") && (n==index))
					return val.id
				n++
			}
		}
		...
		*/
		Call(DomainAndMethod, Params:="", WaitForResponse:=True)
		{
			if !this.Connected || !Online()
			{
				if !WinExist("ahk_exe chrome.exe")
					ExitApp
				MsgBox,,Not connected,Not connected to tab! Please Check Connection.., 10
				Reload
			...
			att=0
			while !this.responses[ID] && ++att<50
				Sleep, 100
